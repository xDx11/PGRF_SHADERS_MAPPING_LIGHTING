...
*/
			...
			private void loadOBJModel(String modelPath) {
				try {
					...
					while ((line = br.readLine()) != null) {
						...
						} else if (line.startsWith("v ")) { // Read in Vertex
															// Data
							vData.add(processData(line));
						} else if (line.startsWith("vt ")) { // Read Texture
																// Coordinates
							vtData.add(processData(line));
						} else if (line.startsWith("vn ")) { // Read Normal
																// Coordinates
							vnData.add(processData(line));
						} else if (line.startsWith("f ")) { // Read Face (index)
															// Data
			...
			private void processfIntData(String sdata[]) {
				...
				for (int loop = 1; loop < sdata.length; loop++) {
					...
					if (loop>3) {	//make a new triangle as a triangle fan
						...
						vdataN[1] = vdata[2]; //second vertex is previous the third one
						vtdataN[1] = vtdata[2];
						vndataN[1] = vndata[2];
						index = 2;
	...
	private OGLBuffers toOGLBuffers(GL2 gl, FloatBuffer verticesBuf, FloatBuffer normalsBuf, FloatBuffer texCoordsBuf){
		OGLBuffers buffers;
